...
export class SequelizeSimpleCache {
  ...
  constructor(config = {}, options = {}) {
    ...
    const {
      ...
      delegate = (event: any, details: any) => console.debug(`CACHE ${event.toUpperCase()}`, details),
    }: any = options;
    this.debug = debug;
    this.ops = ops;
  ...
  init(model: any) { // Sequelize model object
    ...
    return new Proxy(model, {
      get: (target: any, prop: any) => {
        ...
        const fn = async (...args: any) => {
          const withinTxn = args.reduce((acc: any, { transaction }: any) => acc || transaction, false);
  ...
  size(...modelnames: any) {
    const types = modelnames.length ? modelnames : Object.keys(this.cache);
    return types
      .filter((type: any) => this.cache[type])
      .reduce((acc: any, type: any) => acc + this.cache[type].size, 0);
  }

  purge(...modelnames: any) {
    const types = modelnames.length ? modelnames : Object.keys(this.cache);
    const now = Date.now();
    types.forEach((type: any) => {
      const cache = this.cache[type];
      if (!cache) return;
      let oldest: any;
      cache.forEach(({ expires }: any, hash: any) => {
  ...
  log(event: any, details: any = {}) {
    ...
    this.delegate(event, {
      ...
      size: Object.entries(this.cache)
        .reduce((acc: any, [type, map]: any) => ({ ...acc, [type]: map.size }), {}),
    });
  }
}