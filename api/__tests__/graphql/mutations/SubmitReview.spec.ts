...
describe('SubmitReview Mutation [api/graphql/mutations/SubmitReview.ts]', () => {
  ...
  it('should accept a valid review on a user with a common team without an already existing review for the subjects state', async() => {
    ...
    const res = await chai.request(server).post('/graphQL').set('JWT', session_u0).send({
      query:
      `mutation {
        submitReview(receiver_id: "${user4._id}", team_id: "${team0._id}", subject_id: "${team0.subject._id}", signifiesInterest: 1, calm: 1, empathy: 1, newIdeas: 1, proactive: 1, crossTeam: 1, managesOwn: 1, eyeContact: 1, workDemands: 1, openToShare: 1, distractions: 1, cooperatively: 1, positiveBelief: 1, resilienceFeedback: 1, influences: 1, clearInstructions: 1, easilyExplainsComplexIdeas: 1) {
          data {
            review {
              submitter_id
              receiver_id
  ...
  it('should reject an unauthenticated user', async() => {
    const res = await chai.request(server).post('/graphQL').set('JWT', 'unittest').send({
      query:
      `mutation {
        submitReview(receiver_id: "${user1._id}", team_id: "${team0._id}", subject_id: "${team0.subject._id}", signifiesInterest: 1, calm: 1, empathy: 1, newIdeas: 1, proactive: 1, crossTeam: 1, managesOwn: 1, eyeContact: 1, workDemands: 1, openToShare: 1, distractions: 1, cooperatively: 1, positiveBelief: 1, resilienceFeedback: 1, influences: 1, clearInstructions: 1, easilyExplainsComplexIdeas: 1) {
          data {
            review {
              submitter_id
              receiver_id
  ...
  it('should reject a review against an unknown user', async() => {
    const res = await chai.request(server).post('/graphQL').set('JWT', session_u0).send({
      query:
      `mutation {
        submitReview(receiver_id: "unittest", team_id: "${team0._id}", subject_id: "${team0.subject._id}", signifiesInterest: 1, calm: 1, empathy: 1, newIdeas: 1, proactive: 1, crossTeam: 1, managesOwn: 1, eyeContact: 1, workDemands: 1, openToShare: 1, distractions: 1, cooperatively: 1, positiveBelief: 1, resilienceFeedback: 1, influences: 1, clearInstructions: 1, easilyExplainsComplexIdeas: 1) {
          data {
            review {
              submitter_id
              receiver_id
  ...
  it('should reject a review against a user when there is no common team', async() => {
    const res = await chai.request(server).post('/graphQL').set('JWT', session_u0).send({
      query:
      `mutation {
        submitReview(receiver_id: "${user3._id}", team_id: "${team0._id}", subject_id: "${team0.subject._id}", signifiesInterest: 1, calm: 1, empathy: 1, newIdeas: 1, proactive: 1, crossTeam: 1, managesOwn: 1, eyeContact: 1, workDemands: 1, openToShare: 1, distractions: 1, cooperatively: 1, positiveBelief: 1, resilienceFeedback: 1, influences: 1, clearInstructions: 1, easilyExplainsComplexIdeas: 1) {
          data {
            review {
              submitter_id
              receiver_id
